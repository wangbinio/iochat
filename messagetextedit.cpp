  ...
  for (int index = 0; index < doc.size(); index++) {
    if (doc[index] == QChar::ObjectReplacementCharacter) {
      ...
      while (indexUrl < count) {
        ...
        if (this->document()->toHtml().
                  contains(msg.content, Qt::CaseSensitive)) {
          indexUrl++;
...
void MessageTextEdit::insertMsgList(QVector<MsgInfo>& list, const QString& flag,
    const QString& text, const QPixmap& pix) {
  MsgInfo msg;
  msg.msgFlag = flag;
  msg.content = text;
  msg.pixmap = pix;
...
QPixmap MessageTextEdit::getFileIconPixmap(const QString& url) {
  ...
  int maxWidth = textSize.width() > FileSize.width()
                   ? textSize.width()
                   : FileSize.width();
...
QString MessageTextEdit::getFileSize(const qint64 size) {
  QString Unit;
  double num;
  if (size < 1024) {
    num = size;
